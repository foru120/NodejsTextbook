...
const exists = (dir) => {
    `
        fs.constants.F_OK: 읽기/쓰기/실행 권한이 있습니다.
...
const mkdirp = (dir) => {
    `
        path.relative(경로1, 경로2): 경로1 -> 경로2 까지의 상대 경로를 반환
        path.normalize: '..' 와 '.' 부분을 처리해서 문자열 경로를 정규화한다. 
...
program
    ...
    .alias('tmpl')
    .option('-n, --name <name>', '파일명을 입력하세요.', 'index')  // <>: 필수, []: 선택
